...
module.exports = {
  postComment: async (req, res) => {
    if (!req.user) {
      req.flash(
        "error-message",
        "Action not allowed! You have to be logged in to be able to make comments on a post."
      );
  ...
  postCommentsToComments: async (req, res) => {
    if (!req.user) {
      req.flash(
        "error-message",
        "Action not allowed! You have to be logged in to be able to make comments on a post."
    ...
    commentReply.comments.body.unshift(reply);
    commentReply.comments.username = req.user.username || req.user.name;
    commentReply.comments.userAvatar = req.user.userImage;
